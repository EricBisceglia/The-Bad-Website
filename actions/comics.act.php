                      ...
                      bool  $show_all_images = false ) : array|null
{
  ...
  for($i = 0; $row = query_row($comic_images); $i++)
  {
    ...
    if($row['i_bonus'] && !$row['i_remake'] && !$row['i_full'])
      $bonus_count++;
    if(!$row['i_remake']&& $row['i_trans'] && !$row['i_full'] && !$row['i_bonus'] && !$row['i_preview'])
      $transcript_count++;
    if($row['i_remake'] && $row['i_trans'] && !$row['i_full'] && !$row['i_bonus'] && !$row['i_preview'])
      $rtranscript_count++;

    // Update the full transcripts
    if(!$row['i_remake'] && !$row['i_preview'] && !$row['i_full'] && !$row['i_bonus'])
    {
    ...
    // Update the full remake transcripts
    if($row['i_remake'] && !$row['i_preview'] && !$row['i_full'] && !$row['i_bonus'])
    {
      if($data['images']['lang'][$i] === 'EN')
        $full_rtranscript_en  = ($full_rtranscript_en)
                              ? $full_rtranscript_en.PHP_EOL.PHP_EOL.$row['i_trans']
                              : $row['i_trans'];
      else
        $full_rtranscript_fr  = ($full_rtranscript_fr)
                              ? $full_rtranscript_fr.PHP_EOL.PHP_EOL.$row['i_trans']
                              : $row['i_trans'];
    }
  }
  ...
  $data['images']['rtranscripts'] = $rtranscript_count;
  ...
  // Add the full remake transcripts to the returned data
  $data['images']['rtranscript_text_en']  = sanitize_output($full_rtranscript_en, preserve_line_breaks: true);
  $data['images']['rtranscript_text_fr']  = sanitize_output($full_rtranscript_fr, preserve_line_breaks: true);
  $data['images']['rtranscript_md_en']    = sanitize_output($full_rtranscript_en, preserve_line_breaks: true,
                                                                                  markdown_line_breaks: true);
  $data['images']['rtranscript_md_fr']    = sanitize_output($full_rtranscript_fr, preserve_line_breaks: true,
                                                                                  markdown_line_breaks: true);
...
function comic_types_get( int     $comic_type_id    = 0,
                          string  $comic_type_slug  = '' ) : array|null
{
  // Sanitize the comic type's id and slug
  $comic_type_id    = sanitize($comic_type_id, 'int');
  $comic_type_slug  = sanitize($comic_type_slug, 'string');

  // Return null if the comic type does not exist
  if(!$comic_type_id && !$comic_type_slug)
    return null;
  if($comic_type_id && !database_row_exists('comic_types', $comic_type_id))
    return null;
  if($comic_type_slug && !database_entry_exists('comic_types', 'slug', $comic_type_slug))
    return null;
                            ...
                            bool   $is_minor  = false  ) : array
{
  ...
  for($i = 0; $row = query_row($comic_types); $i++)
  {
    ...
    $data[$i]['slug']       = sanitize_output($row['ct_slug']);
...
function comic_types_add( array $data ) : void
{
  ...
  // Determine the slug
  $comic_type_slug = str_replace(' ', '_', $comic_type_name_en);
  $comic_type_slug = preg_replace('/[^a-zA-Z0-9_]/', '', $comic_type_slug);
  $comic_type_slug = string_change_case($comic_type_slug, 'lowercase');
  $comic_type_slug = sanitize($comic_type_slug, 'string');

  // Add the comic type to the database
  query(" INSERT INTO comic_types
          SET         comic_types.sorting_order   = '$comic_type_sort'      ,
                      comic_types.slug            = '$comic_type_slug'      ,
                           ...
                           array $data     ) : void
{
  ...
  // Update the slug
  $comic_type_slug = str_replace(' ', '_', $type_name_en);
  $comic_type_slug = preg_replace('/[^a-zA-Z0-9_]/', '', $comic_type_slug);
  $comic_type_slug = string_change_case($comic_type_slug, 'lowercase');
  $comic_type_slug = sanitize($comic_type_slug, 'string');
